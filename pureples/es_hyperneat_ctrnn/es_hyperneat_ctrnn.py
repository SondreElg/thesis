...
class ESNetworkCTRNN:
    ...
    def create_phenotype_network(self, filename=None):
        ...
        output_nodes = list(
            range(len(input_nodes), len(input_nodes) + len(output_coordinates))
        )
    ...
    def division_initialization(self, coord, outgoing):
        ...
        while q:
            ...
            p.cs[0] = QuadPoint(
                p.x - p.width / 2.0, p.y - p.width / 2.0, p.width / 2.0, p.lvl + 1
            )
            p.cs[1] = QuadPoint(
                p.x - p.width / 2.0, p.y + p.width / 2.0, p.width / 2.0, p.lvl + 1
            )
            ...
            for c in p.cs:
                c.w = query_cppn(
                    coord, (c.x, c.y), outgoing, self.cppn, self.max_weight
                )
    ...
    def pruning_extraction(self, coord, p, outgoing):
        ...
        for c in p.cs:
            ...
            if self.variance(c) > self.variance_threshold:
                ...
                con = None
                if (
                    max(min(d_top, d_bottom), min(d_left, d_right))
                    > self.band_threshold
                ):
                    if outgoing:
    ...
    def clean_net(self, connections):
        ...
        true_nodes -= set(self.substrate.input_coordinates).union(
            set(self.substrate.output_coordinates)
        )
