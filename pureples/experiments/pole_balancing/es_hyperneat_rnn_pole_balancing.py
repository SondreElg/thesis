...
def run(gens, env, version):
    ...
    """
    winner, stats = run_es_rnn(gens, env, 500, CONFIG, params(version), SUBSTRATE)
    print(f"es_hyperneat_rnn_polebalancing_{VERSION_TEXT} done")
    return winner, stats
...
if __name__ == "__main__":
    ...
    NET = NETWORK.create_phenotype_network(
        filename=f"pureples/experiments/pole_balancing/results/es_hyperneat_rnn_pole_balancing_{VERSION_TEXT}_winner"
    )
    draw_net(
        CPPN,
        filename=f"pureples/experiments/pole_balancing/results/es_hyperneat_rnn_pole_balancing_{VERSION_TEXT}_cppn",
    )
    with open(
        f"pureples/experiments/pole_balancing/results/es_hyperneat_rnn_pole_balancing_{VERSION_TEXT}_cppn.pkl",
        "wb",
    ) as output:
